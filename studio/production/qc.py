...
def main():
 ...
 try: rdata=json.loads(Path("output/current/render_manifest.json").read_text(encoding="utf-8")); expected=tuple(rdata.get("resolution",[]))
 except Exception: rdata={}; expected=()
 if not v: failures.append("missing video stream")
 elif expected and (v.get("width"),v.get("height"))!=expected: failures.append(f"wrong resolution: {v.get('width')}x{v.get('height')}, expected {expected[0]}x{expected[1]}")
 ...
 try:
  ...
  registered={x.get("asset") for x in assets}
  ...
  if not Path("output/current/evidence_ledger.json").exists(): failures.append("evidence ledger missing")
 except Exception as e: failures.append(f"manifest/timeline verification error: {e}")
 script=Path("output/current/script.txt")
 if not script.exists() or len(script.read_text(encoding="utf-8",errors="ignore").split())<20: failures.append("narration script missing or too short")
 elif any(x in script.read_text(encoding="utf-8",errors="ignore") for x in ["OFFICIAL SCREENSHOT","OFFICIAL ASSET","ASSET ID","DEBUG","HASH:"]): failures.append("internal provenance/debug label in narration script")
 result={"status":"PASS" if not failures else "FAIL","artifact":str(video),"duration_seconds":duration,"failures":failures,"gates":{"technical":not any("resolution" in x or "video stream" in x for x in failures),"audio":not any("audio" in x for x in failures),"black_frames":not any("black" in x for x in failures),"provenance":not any("provenance" in x or "timeline asset" in x for x in failures),"creative_diversity":not any("diversity" in x for x in failures),"evidence":not any("evidence" in x for x in failures)}}
