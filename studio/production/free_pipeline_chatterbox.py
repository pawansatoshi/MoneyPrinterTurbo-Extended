"""Free-first PawanStudio pipeline using MIT Chatterbox creator voice."""
...
def write_script(language,duration):
 run([sys.executable,str(Path(__file__).with_name("script_writer.py")),str(OUT/"research.json"),language,str(OUT/"script.txt"),str(duration)],check=True)
def capture(pages):
 ...
 pj=OUT/"pages.json"; pj.write_text(json.dumps(pages),encoding="utf-8"); run([sys.executable,str(js),str(pj),str(ASSETS)],check=True); stamp=time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()); reg=[]
...
def voice(language):
 ...
 run(["ffmpeg","-y","-i",str(ref),"-ar","24000","-ac","1",str(refwav)],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 run([sys.executable,str(Path(__file__).with_name("chatterbox_voice.py")),str(OUT/"script.txt"),str(refwav),str(out),language],check=True)
 return out
...
def main():
 ...
 rows,pages=research(a.project_url); run([sys.executable,str(Path(__file__).with_name("evidence_ledger.py")),str(OUT/"research.json"),str(OUT/"evidence_ledger.json")],check=True); write_script(a.language,a.duration); reg=capture(pages); cards=make_visuals(reg); audio=voice(a.language); subs=subtitles(audio,a.language); m=manifest(cards,audio,subs,a.aspect_ratio,a.duration,a.style)
