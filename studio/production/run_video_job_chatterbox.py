"""GitHub Actions entry point for the free Chatterbox PawanStudio factory."""
...
def main():
 ...
 request={"project_url":a.project_url,"language":a.language,"aspect_ratio":a.aspect_ratio,"style":a.style,"duration_seconds":int(a.duration),"free_mode":bool(a.free_mode),"voice_backend":"chatterbox_multilingual","fail_closed":os.getenv("PAWANSTUDIO_FAIL_CLOSED")=="true"}; (OUT/"request.json").write_text(json.dumps(request,indent=2),encoding="utf-8")
 cmd=[sys.executable,str(Path(__file__).with_name("free_pipeline_chatterbox.py")),"--project-url",a.project_url,"--language",a.language,"--aspect-ratio",a.aspect_ratio,"--style",a.style,"--duration",a.duration]
 run(cmd,check=True)
