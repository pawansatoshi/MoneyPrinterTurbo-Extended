"""Free-first PawanStudio production pipeline: research -> official capture -> creator voice -> word subtitles -> render."""
...
ROOT=Path(__file__).resolve().parents[2]; OUT=ROOT/"output/current"; ASSETS=OUT/"official_assets"
...
def discover_pages(url,limit=6):
    ...
    r=requests.get(url,timeout=20,headers={"User-Agent":"PawanStudio/1.0 official-source-research"}); r.raise_for_status()
    soup=BeautifulSoup(r.text,"html.parser"); links=[url]
    for a in soup.select("a[href]"):
        u=urljoin(url,a.get("href")); q=urlparse(u); u=u.split("#",1)[0]
...
def research(url):
    ...
    for page in pages:
        try:
            r=requests.get(page,timeout=20,headers={"User-Agent":"PawanStudio/1.0 official-source-research"}); r.raise_for_status()
...
def make_script(rows,language):
    title=next((r.get("title") for r in rows if r.get("title")),"The project")
    facts=[]
    for r in rows:
    ...
    if language.lower()=="english":
        lines=[f"What is {title}, and why does it matter?"]+facts[:5]+["The important part is how the product actually works, what it lets people do, and what risks or limitations they should understand.","Before using any financial product, verify the current terms and official documentation for yourself."]
    elif language.lower()=="hindi":
        lines=[f"Aaj hum samjhenge {title} kya hai, aur ye kyun important ho sakta hai."]+facts[:5]+["Kisi bhi financial product ko use karne se pehle current terms aur official documentation ko khud verify karna zaroori hai."]
    else:
        lines=[f"What is {title}, aur ye important kyun hai?"]+facts[:5]+["Headline se zyada important hai ki product kaise kaam karta hai aur risks kya hain."]
...
def capture(pages):
    ...
    pj=OUT/"pages.json"; pj.write_text(json.dumps(pages),encoding="utf-8")
    subprocess.run([sys.executable,str(js),str(pj),str(ASSETS)],check=True)
    stamp=time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()); reg=[]
    ...
    (OUT/"asset_manifest.json").write_text(json.dumps(reg,indent=2),encoding="utf-8"); return reg

def voice(script,language):
    ...
    refwav=OUT/"voice_reference.wav"; out=OUT/"narration.wav"
    subprocess.run(["ffmpeg","-y","-i",str(ref),"-ar","22050","-ac","1",str(refwav)],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
def manifest(reg,audio,subs,aspect,duration):
    res={"16:9":[1920,1080],"1:1":[1080,1080],"9:16":[1080,1920]}[aspect]; imgs=[x["asset"] for x in reg]; per=max(3.0,float(duration)/max(1,len(imgs))); cams=["push_in","pan_right","pan_left","drift_left","drift_right","pull_out"]
    scenes=[{"asset":str(ROOT/x),"duration":per,"camera":cams[i%len(cams)],"transition":"fade","crop":"cover"} for i,x in enumerate(imgs)]
    m={"resolution":res,"fps":30,"bitrate":"9000k","audio_bitrate":"256k","crf":18,"seed":42,"scenes":scenes,"audio":str(audio),"enhanced_subtitle":str(subs),"subtitles":{"font_size":54 if aspect=="16:9" else 50,"bottom_margin":90}}
...
def main():
    ...
    rows,pages=research(a.project_url); make_script(rows,a.language); reg=capture(pages); audio=voice(OUT/"script.txt",a.language); subs=subtitles(audio,a.language); m=manifest(reg,audio,subs,a.aspect_ratio,a.duration)
