...
def write_script(language,duration):
 ...
 subprocess.run([sys.executable,str(Path(__file__).with_name("script_writer.py")),str(OUT/"research.json"),language,str(OUT/"script.txt"),str(duration)],check=True,cwd=ROOT,env=env)
def capture(pages):
 ...
 pj=OUT/"pages.json"; pj.write_text(json.dumps(pages),encoding="utf-8"); subprocess.run([sys.executable,str(js),str(pj),str(ASSETS)],check=True,cwd=ROOT); stamp=time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()); reg=[]
...
def voice(language):
 ...
 subprocess.run([sys.executable,"-c",code,str(OUT/"script.txt"),str(out),str(refwav),lang],check=True,cwd=ROOT); return out
def subtitles(audio,language):
 out=OUT/"enhanced_subtitles.json"; subprocess.run([sys.executable,str(Path(__file__).with_name("subtitle_sync.py")),str(audio),str(out),{"english":"en","hindi":"hi","hinglish":"en"}.get(language.lower(),"en")],check=True,cwd=ROOT); return out
...
def main():
 ...
 rows,pages=research(a.project_url); subprocess.run([sys.executable,str(Path(__file__).with_name("evidence_ledger.py")),str(OUT/"research.json"),str(OUT/"evidence_ledger.json")],check=True,cwd=ROOT); write_script(a.language,a.duration); reg=capture(pages); cards=make_visuals(reg); audio=voice(a.language); subs=subtitles(audio,a.language); m=manifest(cards,audio,subs,a.aspect_ratio,a.duration,a.style)
